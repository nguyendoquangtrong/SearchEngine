...
class DatabaseBuilder:
    ...
    def build_vector_db(self, cleaned_data):
        ...
        for col in ["image_clip_collection", "text_clip_collection", "text_sbert_collection", "image_caption_sbert_collection"]:
        ...
        img_cap_collection = chroma_client.create_collection("image_caption_sbert_collection")
        ...
        print("\n🖼️ Đang quét đường dẫn Ảnh và Captions...")
            ...
            if target_folder:
                ...
                if os.path.exists(caption_file):
                    with open(caption_file, 'r', encoding='utf-8') as cf:
                        for line in cf:
                            if ' | ' in line:
                                f_name, c_text = line.split(' | ', 1)
                                f_name = f_name.strip()
                                c_text = c_text.strip()
                                if c_text:
                                    caption_docs.append(c_text)
                                    caption_metas.append({"movie_name": title_goc, "file_name": f_name, "type": "caption"})
                                    caption_ids.append(f"cap_{normalize_name(title_goc)}_{len(caption_docs)}")
                                    cap_count += 1
            
            print(f"  [{i+1:03d}/{total_clean}] Quét ảnh: '{title_goc}' -> Tìm thấy {img_count} frames, {cap_count} captions")
        ...
        print(f"\n📝 Đang nén SBERT cho {len(caption_docs)} Captions...")
        total_cap_batches = math.ceil(len(caption_docs) / BATCH_SIZE)
        
        for i in range(0, len(caption_docs), BATCH_SIZE):
            b_docs = caption_docs[i:i+BATCH_SIZE]
            b_vecs = self.sbert_model.encode(b_docs, show_progress_bar=False).tolist()
            img_cap_collection.add(embeddings=b_vecs, documents=b_docs, metadatas=caption_metas[i:i+BATCH_SIZE], ids=caption_ids[i:i+BATCH_SIZE])
            
            if (i // BATCH_SIZE + 1) % 5 == 0 or (i // BATCH_SIZE + 1) == total_cap_batches: 
                print(f"  -> Đã lưu batch caption {i // BATCH_SIZE + 1}/{total_cap_batches} vào Database...")
